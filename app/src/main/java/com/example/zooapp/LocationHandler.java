...
public class LocationHandler {
        ...
        var locationListener = new LocationListener() {
            ...
            public void onLocationChanged(@NonNull Location location) {
                ...
                if (directionsActivity.isBackwards()) {
                    directionsActivity.getSetDirections().setGraphPath(directionsActivity.getAlgorithm().runReversePathAlgorithm(directionsActivity.getExhibitLocations()
                                    .getZooNodeClosestToCurrentLocation(directionsActivity.getLocationToUse()),
                            directionsActivity.getUserListShortestOrder().get(directionsActivity.getCurrIndex() + 1)));
    ...
    public void handleLocationSkip() {
        ...
        var nearestZooNode =
                directionsActivity.getExhibitLocations().getZooNodeClosestToCurrentLocation(getLocationToUse());
        ...
        Log.d("Check Location", "New Graph Path: " + reorderedExhibits.toString());
        var originalVisitedExhibits =
                directionsActivity.getSetDirections().getGraphPaths().subList(0, directionsActivity.currIndex);
        Log.d("Check Location", "Old Graph Path: " + originalVisitedExhibits);
        directionsActivity.getSetDirections().setGraphPaths(Stream.concat(originalVisitedExhibits.stream(),
                reorderedExhibits.stream()).collect(Collectors.toList()));
        ...
        // Find the new path to display
        directionsActivity.getSetDirections().setGraphPath(directionsActivity.algorithm.runPathAlgorithm(nearestZooNode,
                directionsActivity.getExhibitLocations().exhibitsSubList));
    }
